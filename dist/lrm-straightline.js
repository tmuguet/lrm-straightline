...
L.Routing.StraightLine = L.Evented.extend({
  ...
  _routeBetween: function _routeBetween(from, to, interval) {
  ...
  route: function route(waypoints, callback, context) {
    ...
    for (var _i = 0; _i < waypoints.length - 1; _i += 1) {
      var info = this._routeBetween(waypoints[_i].latLng, waypoints[_i + 1].latLng, _options.interval);

      instructions.push({
        type: 'Straight',
        text: "Azimuth ".concat(Math.round(info.azimuth)),
      ...
      for (var j = 0; j < info.route.length; j += 1) {
